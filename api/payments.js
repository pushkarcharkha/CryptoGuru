...
app.post('/api/payments/create-stripe-session', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.APP_BASE_URL || 'https://cryptoguru.ai'}/app?payment=success&plan=${plan}`,
      cancel_url: `${process.env.APP_BASE_URL || 'https://cryptoguru.ai'}/payment/${plan}?billing=${billing}&payment=cancelled`,
    });
