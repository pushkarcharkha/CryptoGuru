    ...
    const handleTaskComplete = (success: boolean) => {
        if (success) {
            ...
            if (nextAttempts >= 2) {
                ...
                setTaskSuccess(true); // Mark as "pseudo-success" to allow proceeding
                setUserFeedback("Learning Opportunity: We've highlighted the correct zone for you. Study the placement before proceeding.");
            } else {
                setUserFeedback(`Incorrect. Try again (1 attempt left)`);
            }
        }
    };
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, background: '#07070c', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '20px', left: '20px', zIndex: 10, pointerEvents: 'none' }}>
                        <div style={{ background: 'rgba(11, 11, 20, 0.8)', padding: '8px 16px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)' }}>
                            <div style={{ fontSize: '10px', color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Terminal Environment</div>
                            <div style={{ fontSize: '14px', fontWeight: 800, color: '#fff' }}>BTC/USDT <span style={{ color: '#10b981', fontSize: '10px', fontWeight: 600, marginLeft: '6px', opacity: 0.8 }}>LIVE SIM</span></div>
                        </div>
                    </div>

                    <div style={{ height: '100%', width: '100%', padding: '24px', paddingTop: '80px', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ flex: 1, minHeight: 0 }}>
                            <LearningChart 
                                data={lesson.simulation.snapshotData} 
                                task={step === 'task' && !revealAnswer ? lesson.chartTasks[0] : undefined}
